...
class DaoQuestionTest {
    ...
    @Test
    @DisplayName("PART 1: Search question by topic: TEST 3: If topic doesn't exist we cannot get data," +
            " from it, so we return empty list")
    void if_table_doesnt_exist_return_empty_list() {
        assertEquals(EMPTY_LIST, interactWithDb.showAllQuestionsByTopic("Cars"));
    }

    @Test
    @DisplayName("PART 1: Search question by topic: TEST 4: If topic does exist we get data from it," +
